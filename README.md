...
- `train/weights/best.pt` — финальная и лучшая модель  
...
```bash
yolo detect train model="runs/detect/train/weights/best.ptc data=cwfid.yaml epochs=150 imgsz=640
```
...
Интересно
```bash
yolo detect predict model="runs/detect/train/weights/best.pt" source="dataset/images/val" save=True
```
